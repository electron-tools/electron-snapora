...
- `V/R/O/A/P/T/M` 切换工具，`Ctrl/Command + C`、`Enter` 或有效选区内双击复制并完成，`Ctrl/Command + S` 保存，`Escape` 取消；按钮保留键盘焦点态和 `aria-label`。
- 主题使用“基础色阶、公开语义 Token、内部组件 Token”三层结构，支持暗色/亮色模式，并允许宿主覆盖强调色、遮罩、工具栏、气泡、危险、warning 和选区控制点颜色；复制成功 Toast 使用 warning 色图标与边框，遮罩消失后显示 3 秒，窗口全程鼠标穿透且不延迟截图结果；动效遵守 `prefers-reduced-motion`。
- 框选开始前使用包内 24px 矢量准星：白色细线负责精度、深色外描边保证明暗背景对比，蓝色中心点表达截图准备状态；热点固定在中心，选区完成后恢复各标注工具自身的光标语义。
...
- macOS 在 `not-determined` 时由首次真实采集触发系统授权弹窗；已拒绝时打开屏幕录制隐私设置并返回 `PERMISSION_DENIED`，受系统策略限制时只返回错误。
